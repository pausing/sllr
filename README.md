...
Production web application for capturing, classifying, storing, automating, and reusing lessons learned. Built with **FastAPI**, **React**, and **SQLite** for the live lesson store (CSV for import/export only).

## Architecture

- **Backend**: FastAPI 0.115+, Uvicorn, Python 3.12, Pydantic v2
- **Frontend**: Vite 8, React 19, React Router 7, TypeScript, Tailwind CSS 4
- **Data**: SQLite `lessons.db` under `SLLR_DATA_DIR` (or `data/` in the image). Reference vocabularies stay as CSVs in `data/`.
...
- `SLLR_DATA_DIR`: Directory for `lessons.db` (default: `data` in the image). Set to `/data` only after the volume can hold a SQLite file; an empty directory is OK. Do not require CSVs on the volume.
...
- `GET /sllr/api/export/csv` — Export live lessons as `lessons_master.csv`
- `POST /sllr/api/import/csv` — Replace live SQLite from uploaded CSV
...
Lesson schema defined in `src/sllr/config.py`. Required fields:
...
Connect Power BI to a CSV export (`GET /sllr/api/export/csv` or `GET /sllr/api/reports/dashboard-export`). See `docs/POWER_BI_INTEGRATION.md`.
...
backend/app/              # FastAPI application
  ...
  storage.py              # SQLite live store wrapper
  identity.py             # Portal ForwardAuth Owner headers
...
data/                     # Reference vocab CSVs; lessons.db created at runtime
...
- SQLite is the live source of truth; CSV is import/export
- SQLite WAL + a process lock prevent torn writes
...
- Dark theme (portal/pvDesign): ink #0c0e12, panel #141820, raised #1b212c, line #2a3140, text #e8eaef, muted #8b93a7, accent #3dcc8c
